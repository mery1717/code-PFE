...
<?php include '../codes_vues/hea.php' ?>
  <hr style="background: gray; color :black;height: 5px; margin-bottom: 0px;margin-top: 0px;">
      ...
      <hr width="50%">
        <center><h3 style="font-weight: bold;color: black;font-size: 30px">Table des sous familles</h3>
      <hr width="50%">            
        </center>
       <div class="container">
         <center>
...
<div class="example">
      <table id="editing-example" class="table table-striped" data-paging="true" data-filtering="true" data-sorting="true" style="width: 70%;">
        ...
        <tr>
          ...
          <th data-name="idSousFamille" data-breakpoints="xs" data-type="number" scope="col">idSousFamille</th>
          <th data-name="famille" scope="col">Famille</th>
          <th data-name="sousFamille" scope="col">Sous famille</th>
      ...
      <div class="container" style="margin-top:35px">
        <button type="button" class="btn btn-default btn-sm" id="ajouterSousFamille" style="background-color: green; color: white;">
          <span class="glyphicon glyphicon-paperclip" style="color: white;"></span>&nbsp; Ajouter sous famille
        </button>
      </div>
    ...
    <form method="POST" action="<?php echo $_SERVER['PHP_SELF'] ;?>" id="ajouter">
                              ...
                              <label for="idFamille">Famille :</label>
                                ...
                                <input type="text"  name = "SousFamille" required="true" />
...
<script type="text/javascript">
  function viewData(){
        $.ajax({
            url: 'sousFamille_data.php?p=view',
            method: 'GET'
        }).done(function(data){
            $('tbody').html(data)
            tableData()
    ...
    function tableData(){
        $('#editing-example').Tabledit({
            ...
            columns: {
                identifier: [0, 'idSousFamille'],
                editable: [[2, 'sousFamille']]
            },
...
//ajouter marque
(function() {
    var updateButton = document.getElementById('ajouterSousFamille');
...
if(isset($_POST['ajouter']))
{
 ...
 //   <?php
 // session_start() ;
  if(!empty($_POST['idFamille']) AND !empty($_POST['SousFamille']))
  {
  
   include('C:\wamp64\www\codePFE\models\connexionBD_model.php');
   
   $sql2 ="SELECT * from familles_sousfamilles where idFamille='".$_POST['idFamille']."'AND idSousFamille=(SELECT idSousFamille FROM sousfamilles where sousFamille='".$_POST['SousFamille']."')";
   
    $resultat2 =$bdd->query($sql2) ;
    if($datas=$resultat2->fetch()) { ?>
     <script>
        alert("Erreur: Sous famille déjà existé!");
      </script>    ';
    <?php }
    else{ 
  $p = $bdd->prepare(' INSERT INTO sousfamilles (sousFamille)
     VALUES (?)');
     $p->execute([$_POST['SousFamille']]);
     $idSousFamille=$bdd->lastInsertId();
     $c = $bdd->prepare(' INSERT INTO familles_sousfamilles (idFamille,idSousFamille)
     VALUES (?,?)');
     $c->execute([$_POST['idFamille'],$idSousFamille]);
