...
<?php include '../codes_vues/hea.php' ?>
  <hr style="background: gray; color :black;height: 5px; margin-bottom: 0px;margin-top: 0px;">
      ...
      <hr width="50%">
        <center><h3 style="font-weight: bold;color: black;font-size: 30px">Table des familles</h3>
      <hr width="50%">            
        </center>
       <div class="container">
         <center>
...
<div class="example">
      <table id="editing-example" class="table table-striped" data-paging="true" data-filtering="true" data-sorting="true" style="width: 70%;">
        <thead>
        <tr>
          <th data-name="idFamille" data-breakpoints="xs" data-type="number" scope="col">idFamille</th>
      ...
      <div class="container" style="margin-top:35px">
        <button type="button" class="btn btn-default btn-sm" id="ajouterFamille" style="background-color: green; color: white;">
          <span class="glyphicon glyphicon-paperclip" style="color: white;"></span>&nbsp; Ajouter famille 
        </button>
      </div>

    <dialog id="zeroDialog" >
    <form method="POST" action="<?php echo $_SERVER['PHP_SELF'] ;?>" id="ajouter" >
     <label for="ajout">Veuillez saisir la nouvelle famille:</label>
        <input type="text"  name = "famille" required="true" />
...
(function() {
    var updateButton = document.getElementById('ajouterFamille');
...
if(isset($_POST['ajouter']))
{
 ...
 // session_start() ;
  if(!empty($_POST['famille']))
  {
   ...
   $sql2 ="SELECT * FROM familles where famille ='".$_POST['famille']."'" ;
    $resultat2 =$bdd->query($sql2) ;
    if($datas=$resultat2->fetch()) { ?>
     <script>
        alert("Cette famille existe déj!");
      </script>    ';
    <?php }
    else{
  $p = $bdd->prepare(' INSERT INTO familles (famille)
     VALUES (?)');
     $p->execute([$_POST['famille']]);
     echo '
    <script>
        alert("la nouvelle famille a bien été enregistré!");
      </script>    ';
    }}
    else echo '
    <script>
        alert("la nouvelle famille n\'a pas bien été enregistré, Veuillez reéssayer plut tard!");
      </script>    ';
  }
