...
session_start() ;
  ...
  $sql= "SELECT * FROM users WHERE users.PPR ='".$_SESSION['PPR']."' ";
...
<?php 
 ...
 {     
          ...
          elseif ($_POST['mot_pass']!=$_POST['mot_pass_con']) {
            ...
            $stmt = $bdd->prepare('UPDATE `users` SET `nom`="'.$nom.'",`prenom`="'.$prenom.'",`login`="'.$login.'",`password`="'.$password.'",`profile`="'.$profile.'",`adresseMail`="'.$email.'" WHERE PPR="'.$_SESSION['PPR'].'"');
        $stmt->execute();
